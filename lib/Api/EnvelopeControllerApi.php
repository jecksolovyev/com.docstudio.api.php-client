    ...
    protected function quickSendExternalDocumentsRequest($files, $data, $mailbox)
    {
        ...
        if ($files !== null) {
            ...
            if (is_array($files)) {
                for ($i = 0; $i < count($files); $i++) {
                    $preparedFiles[] = \GuzzleHttp\Psr7\try_fopen(ObjectSerializer::toFormValue($files[$i]), 'rb');
                }
            } else {
                $preparedFiles = [\GuzzleHttp\Psr7\try_fopen(ObjectSerializer::toFormValue($files[$i]), 'rb')];
            }
            $formParams['files'] = $preparedFiles;
        }
        // form params
        if ($data !== null) {
        ...
        if (isset($_tempBody)) {
            ...
            if ($multipart) {
                $multipartContents = [];
                foreach ($formParams as $formParamName => $formParamValue) {
                    if (is_array($formParamValue) && gettype(reset($formParamValue)) === 'resource') {
                        for ($i = 0; $i < count($formParamValue); $i++) {
                            $multipartContents[] = [
                                'name' => $formParamName,
                                'contents' => $formParamValue[$i]
                            ];
                        }
                    } else {
                        $multipartContents[] = [
